...
<head>
    ...
    <title>Table administrateurt</title>
</head>
<body>
<div class="container">
     ...
     <div class="container col-md-12">
        <table class="table table-bordered">
          <thead class="thead-light">
            <tr>
              <th><a href="formAdmin.php"><button type="button" class="btn btn-primary">
              Insert</button></a></th>
            </tr>
          </thead>
        </table>
     ...
     <div class="container col-md-12">
        <table class="table table-bordered">
          <tr class='bg-info'>
            ...
            <th>Téléphone</th><th>E-mail</th><th>Login</th><th>Mot de passe</th>
          ...
          <?php foreach($rows as $row): ?>
            <tr>
              ...
              <td><?php echo $row['PrenomAdmin'] ?></td>
              <td><?php echo $row['AdresseAdmin'] ?></td>
              <td><?php echo $row['TelephoneAdmin'] ?></td>
              <td><?php echo $row['EmailAdmin'] ?></td>
              <td><?php echo $row['login'] ?></td>
              <td><?php echo $row['password'] ?></td>
              <td><a onclick="return confirm('Etes-vous sur de vouloir faire des modifications?');"
               href="updateAdmin.php?IdAdmin=<?php echo $row['IdAdmin'] ?>"><button type="button" class="btn btn-success">update</button></a></td>
              <td><a onclick="return confirm('Voulez-vous supprimer l\'administrateur?');"
               href="deleteAdmin.php?IdAdmin=<?php echo $row['IdAdmin'] ?>"><button type="button" class="btn btn-danger">delete</button></a></td>
            </tr>
          <?php endforeach; ?> 
        </table>
     </div>
